...
export default function Dashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      if (err.response?.status === 400) alert("Connect Spotify first!");
    }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      ...
      <button onClick={connectSpotify} className="bg-green-500 hover:bg-green-600 px-4 py-2 rounded-lg mb-4">Connect Spotify</button>
      <button onClick={loadData} className="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-lg mb-4 ml-2">Refresh</button>
      <button onClick={logout} className="bg-red-500 hover:bg-red-600 px-4 py-2 rounded-lg mb-4 ml-2">Logout</button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {playlists.map((pl) => (
            <div key={pl.id} className="bg-gray-800/80 hover:bg-gray-800 p-4 rounded-xl shadow-lg transition-colors">
              {pl.images?.[0]?.url ? <img src={pl.images[0].url} className="w-full aspect-square object-cover rounded-lg mb-3" /> : <div className="w-full aspect-square rounded-lg bg-gray-700 mb-3" />}
        ...
        <div className="bg-gray-800/80 rounded-xl divide-y divide-white/10">
          {liked.slice(0, 10).map((it) => (
            <div key={it.track?.id} className="flex items-center gap-3 p-3">
              <img src={it.track?.album?.images?.[2]?.url} className="w-12 h-12 rounded" />
