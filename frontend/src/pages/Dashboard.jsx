...
export default function Dashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      const res = await api.get("/spotify/liked");
      setLiked(res.data || []);
  ...
  const connectSpotify = () => {
    ...
    window.location.href = `${backendURL}/spotify/login?token=${token}`;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={connectSpotify} className="bg-green-500 hover:bg-green-600 px-4 py-2 rounded-lg">Connect Spotify</button>
        <button onClick={loadData} className="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-lg">Refresh</button>
