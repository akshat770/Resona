class PlaybackService {
  constructor() {
    ...
    this.currentQueue = []; // ADDED: Track current queue
    this.currentIndex = 0;   // ADDED: Track current position
  }
  ...
  // UPDATED: Play track with queue context
  async playTrack(trackUri, previewUrl = null, queue = null, index = 0) {
    console.log('PlayTrack called:', { trackUri, previewUrl, isPremium: this.isPremium });
    
    // ADDED: Set queue context if provided
    if (queue) {
      this.currentQueue = queue;
      this.currentIndex = index;
      console.log('Queue set:', { queueLength: queue.length, currentIndex: index });
    } else {
      // Single track - create minimal queue
      this.currentQueue = [trackUri];
      this.currentIndex = 0;
    }
    ...
    if (this.isPremium && this.deviceId && this.accessToken) {
      try {
        ...
        // If we have a queue, play the entire context
        if (queue && queue.length > 1) {
          ...
          if (response.ok || response.status === 204) {
            console.log('Premium: Queue started on web player');
            return;
          }
        } else {
          // Single track
